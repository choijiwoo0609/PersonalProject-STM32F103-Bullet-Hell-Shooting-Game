...
//volatile int plane_bullet_timer = 0;    // 총알 발사 주기를 세는 타이머.
//volatile int plane_bullet_ready = 0;    // 총알 발사 준비 완료 표시.

void SysTick_Handler(void)
{
	SysTick_Flag = 1;
  #if 0
  plane_bullet_timer += 1;

  if(plane_bullet_timer >= 20){   // 10 * 20 = 200[ms]마다 Interrupt가 발생.
    plane_bullet_timer = 0;       // 발사 타이머 초기화.
    plane_bullet_ready = 1;       // 발사 준비 완료.
  }
  #endif
}
