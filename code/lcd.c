...
// 화면 전체를 지우는 함수. => 그림을 이동시키는 데에는 비효율적.
...
#if 0
void Lcd_Draw_Line(int x1, int y1, int x2, int y2, unsigned short Color)
{
    if (x1 == x2) {
        // 세로선
        ...
        for (int x = x1; x <= x2; x++) {
            Lcd_Draw_Pixel(x, y1, color);
        }
    }
    // 대각선 등은 필요 시 이후에 추가 가능
}

// x1, x2, x3, y1, y2, y3, color
void Lcd_Draw_Triangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned short Color)
{
    // 세 점을 잇는 선을 그린다.
